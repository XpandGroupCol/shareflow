...
const NavCollapseLink = ({ items = [], basePath, text, icon: Icon, onClose, createPath }) => {
  const { pathname } = useLocation()
  const [open, setOpen] = useState(Boolean(pathname.includes(createPath || basePath)))
      ...
      <Collapse in={open} timeout='auto' unmountOnExit>
        ...
        >
          {items.map(({ to, text }, index) => {
            return (
              <NavItemLink externalActive={pathname.split('/').length > 2 && index !== 0} key={to} to={to} text={text} onClose={onClose} icon={FiberManualRecordIcon} />
            )
          })}
        </List>
      </Collapse>
